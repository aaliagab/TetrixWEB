...
function canRight() {
    return (rotacion == 0 && columna_inicial < 9 &&
        ...
        (rotacion == 2 && columna_inicial < 9 &&
            isVacio(fila_inicial, columna_inicial + 3) &&
            isVacio(fila_inicial + 1, columna_inicial + 1)) ||

        (rotacion == 3 && columna_inicial < 10 &&
            isVacio(fila_inicial, columna_inicial + 1) &&
            isVacio(fila_inicial + 1, columna_inicial + 1) &&
            isVacio(fila_inicial + 2, columna_inicial + 2));
}
...
function canDown() {
    return (rotacion == 0 && fila_inicial < 14 &&
        ...
        (rotacion == 2 && fila_inicial < 14 &&
            isVacio(fila_inicial + 2, columna_inicial) &&
            isVacio(fila_inicial + 1, columna_inicial + 1) &&
            isVacio(fila_inicial + 1, columna_inicial + 2)) ||

        (rotacion == 3 && fila_inicial < 13 &&
...
export function moveDown() {
    if (rotacion == 0) {
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 2)).className = 'cverde';
            fila_inicial++;
        ...
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 3) + '-' + (columna_inicial + 1)).className = 'cverde';
            fila_inicial++;
            if (!canLeft() && !canRight() && !canDown()) {
                iniciarNuevaFigura();
            }
        ...
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 1)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cverde';
            fila_inicial++;
            if (!canLeft() && !canRight() && !canDown()) {
                iniciarNuevaFigura();
            }
        ...
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial + 3) + '-' + (columna_inicial)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 3) + '-' + (columna_inicial + 1)).className = 'cverde';
            fila_inicial++;
            if (!canLeft() && !canRight() && !canDown()) {
                iniciarNuevaFigura();
...
export function moveLeft() {
    if (rotacion == 0) {
        if (canLeft()) {
            ...
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial - 1)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 1)).className = 'cverde';
            columna_inicial--;
...
export function moveRight() {
    if (rotacion == 0) {
        if (canRight()) {
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 3)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 3)).className = 'cverde';
            columna_inicial++;
        ...
        if (canRight()) {
            ...
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 1)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 2)).className = 'cverde';
            columna_inicial++;
            if (fila_inicial == 13 && columna_inicial == 9 && !canLeft()) {
                iniciarNuevaFigura();
            }
        }
    } else if (rotacion == 2) {
...
export function rotar() {
    if (rotacion == 0) {
        if (fila_inicial < 14 &&
            ...
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 1)).className = 'cverde';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cverde';
            rotacion = 1;
        }
