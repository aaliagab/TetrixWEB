...
function canDown() {
    return (rotacion == 1 && fila_inicial < 14 &&
        isVacio(fila_inicial + 1, columna_inicial) &&
        isVacio(fila_inicial + 2, columna_inicial + 1) &&
        isVacio(fila_inicial + 2, columna_inicial + 2)) ||
        (rotacion == 0 && fila_inicial < 13 &&
            isVacio(fila_inicial + 3, columna_inicial) &&
            isVacio(fila_inicial + 2, columna_inicial + 1));
}

//MOVIMIENTOS S INVERTIDA
export function moveDown() {
    if (rotacion == 1) {
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 2)).className = 'cnaranja';
            fila_inicial++;
        ...
        if (canDown()) {
            ...
            document.getElementById('img-' + (fila_inicial + 3) + '-' + (columna_inicial)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cnaranja';
            fila_inicial++;
            if (!canLeft() && !canRight() && !canDown()) {
                iniciarNuevaFigura();
...
export function moveLeft() {
    if (rotacion == 1) {
        if (canLeft()) {
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 1)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial - 1)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial)).className = 'cnaranja';
            columna_inicial--;
        ...
        if (canLeft()) {
            ...
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial - 1)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial - 1)).className = 'cnaranja';
            columna_inicial--;
...
export function moveRight() {
    if (rotacion == 1) {
        if (canRight()) {
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 1)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 3)).className = 'cnaranja';
            columna_inicial++;
        ...
        if (canRight()) {
            ...
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial + 2)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial + 1)).className = 'cnaranja';
            columna_inicial++;
...
export function rotar() {
    if (rotacion == 1) {
        if (fila_inicial < 14 && isVacio(fila_inicial + 1, columna_inicial) &&
            isVacio(fila_inicial + 2, columna_inicial)) {
            document.getElementById('img-' + (fila_inicial) + '-' + (columna_inicial)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial + 2)).className = 'cnone';
            document.getElementById('img-' + (fila_inicial + 1) + '-' + (columna_inicial)).className = 'cnaranja';
            document.getElementById('img-' + (fila_inicial + 2) + '-' + (columna_inicial)).className = 'cnaranja';
            rotacion = 0;
        }
    } else if (rotacion == 0) {
